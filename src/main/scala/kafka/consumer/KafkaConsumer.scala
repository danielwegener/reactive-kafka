 ...
 */
class KafkaConsumer[K : Decoder,V : Decoder](
                     topic: String,
                     /** topic
  ...
  lazy val keyDecoder = implicitly[Decoder[K]]
  lazy val valueDecoder = implicitly[Decoder[V]]

  val filterSpec = new Whitelist(topic)

  info("setup:start topic=%s for zk=%s and groupId=%s".format(topic,zookeeperConnect,groupId))
  val stream = connector.createMessageStreamsByFilter(filterSpec, 1, keyDecoder, valueDecoder)(0)
  info("setup:complete topic=%s for zk=%s and groupId=%s".format(topic,zookeeperConnect,groupId))

  def read(write: ((K,V))=>Unit) = {
    info("reading on stream now")
    for(messageAndTopic <- stream) {
      try {
        info("writing from stream")
        write(messageAndTopic.key() -> messageAndTopic.message())
